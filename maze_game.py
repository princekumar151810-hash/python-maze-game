...
class Ball:
    ...
    def draw(self):
        pygame.draw.circle(screen, RED, (int(self.x + BALL_SIZE/2), int(self.y + BALL_SIZE/2)), BALL_SIZE//2)
...
class Game:
    def __init__(self):
        ...
        self.walls = []
        self.goal = pygame.Rect(WIDTH - CELL_SIZE + 5, HEIGHT - CELL_SIZE + 5 - 40, CELL_SIZE-10, CELL_SIZE-10)
    ...
    def load_maze(self):
        ...
        if maze_img is None:
            ...
            return
        maze_img = cv2.resize(maze_img, (WIDTH, GAME_HEIGHT))
        _, maze_binary = cv2.threshold(maze_img, 127, 255, cv2.THRESH_BINARY)
        wall_padding = 1
        for y in range(GAME_HEIGHT):
            for x in range(WIDTH):
                if maze_binary[y, x] == 0:
                    self.walls.append(Wall(x, y, WALL_SIZE + wall_padding, WALL_SIZE + wall_padding))

    def draw_score(self, maze_completed):
        score_rect = pygame.Rect(0, GAME_HEIGHT, WIDTH, 40)
        pygame.draw.rect(screen, SCORE_BG, score_rect)
        pygame.draw.line(screen, BLACK, (0, GAME_HEIGHT), (WIDTH, GAME_HEIGHT), 2)
        text = self.font.render(f"Mazes Completed: {maze_completed}", True, BLACK)
        text_rect = text.get_rect(center=(WIDTH//2, GAME_HEIGHT + 20))
    ...
    def run(self):
        ...
        while running:
            ...
            self.ball.move(dx, dy, self.walls)

            if self.ball.rect.colliderect(self.goal):
                maze_completed += 1
                print(f"Maze {maze_completed} completed! Loading next maze...")
            ...
            self.draw_score(maze_completed)
